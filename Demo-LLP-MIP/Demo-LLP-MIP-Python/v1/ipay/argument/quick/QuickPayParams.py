...
class QuickPayParams:
    def __init__(self):
        ...
        self.goods_info = None  # 假设 GoodsInfo 为另一个类
        self.risk_info = None  # 假设 RiskInfo 为另一个类
        self.device_info = None  # 假设 DeviceInfo 为另一个类
        self.store_info = None  # 假设 StoreInfo 为另一个类
        self.payee_infos = None  # 假设 PayeeInfo 为另一个类
        self.pay_method_infos = None  # 假设 PayMethodInfo 为另一个类
        self.balance_info = None  # 假设 BalanceInfo 为另一个类
    ...
    def to_dict(self):
        ...
        if self.card_info is not None:
            data["card_info"] = self.card_info.__dict__
        if self.goods_info is not None:
            data["goods_info"] = self.goods_info.__dict__
